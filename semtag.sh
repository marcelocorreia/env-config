...
Commands:
  ...
  final      Tags the current build as a final version, this only can be done on the ${branch} branch.
...
function get_current {
  ...
  if [ "$__commitcount" == "0" ] && [ -z "$__status" ]; then
    ...
    if [ "$__currentbranch" != "${branch}" ]; then
...
case $ACTION in
  ...
  final)
    init
    diff=$(git diff ${branch} | cat)
    if [ "$forcetag" == "false" ]; then
      if [ -n "$diff" ]; then
        echo "ERROR: Branch must be updated with ${branch} for final versions"
        exit 1
      fi
    fi
    increase_version
